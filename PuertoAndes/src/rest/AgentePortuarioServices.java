...
@Path("agentePortuario")
public class AgentePortuarioServices {
	...
	public Response addSalidaBuque(MovimientoBuque salidaBuque) {
		PuertoAndesMaster tm = new PuertoAndesMaster(getPath());
		try {
			tm.addSalidaBuque(salidaBuque);
		} catch (Exception e) {
			return Response.status(500).entity(doErrorMessage(e)).build();
		}
		return Response.status(200).entity(salidaBuque).build();
	}
}
