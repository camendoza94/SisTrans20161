...
CONSTRAINT FK_ID_PUERTO FOREIGN KEY (ID_PUERTO)
...
CONSTRAINT FK_ID_CUARTO FOREIGN KEY (ID_CUARTO)
...
CONSTRAINT CK_TIPO CHECK (TIPO= 'RORO' OR TIPO = 'MULTIPROPOSITO' OR TIPO= 'PORTACONTENEDORES')
);
...
TIPO VARCHAR(255),
CONSTRAINT PK_TIPO PRIMARY KEY (TIPO),
CONSTRAINT CK_TIPO CHECK (TIPO= 'HABITUAL' OR TIPO = 'NO_HABITUAL')
);
...
--Falta contraints de que solo puede estar en alguno de los tres.
...
CONSTRAINT NN_CIUDAD CHECK (CIUDAD IS NOT NULL),
CONSTRAINT NN_NOMBRE CHECK (NOMBRE IS NOT NULL)
);
